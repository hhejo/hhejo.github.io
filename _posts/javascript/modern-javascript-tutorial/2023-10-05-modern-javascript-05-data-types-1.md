...
last_modified_at: 2023-12-05 12:20:24 +0900
...
- 비 엄격 모드: `undefined`
  - 에러가 발생하지 않고 래퍼 객체에 프로퍼티 `test`가 추가됨
...
- `base` 진법으로 `num`을 표현한 후, 문자형으로 변환하고 반환
- `base`는 `2` ~ `36`까지 가능(기본값은 `10`)
...
- 인수를 숫자로 변환하고, 결과가
  - 일반 숫자인 경우 `true` 반환
  - `NaN`, `Infinity`, `-Infinity`이면 `false` 반환
- 문자열이 일반 숫자인지 검증하는 데 사용
- 빈 문자열(`""`)이나 공백만 있는 문자열(`"  "`)
  - `isFinite`를 포함한 모든 숫자 관련 내장 함수에서 `0`으로 취급
