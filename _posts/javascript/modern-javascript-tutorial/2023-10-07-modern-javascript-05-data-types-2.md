...
last_modified_at: 2023-12-05 13:19:44 +0900
...
- 객체형에 속하기 때문에 사용할 수 있으나 배열의 길이는 변하지 않음
- 해당 키에 상응하는 값만 지움
- 빈 공간을 채우지는 않음
...
- 객체가 인자로 넘어오면(유사 배열 객체라도) 객체는 분해되지 않고 통으로 복사
...
객체에 `Symbol.iterator`(특수 내장 심볼) 메서드 추가
...
- 코드가 짧아짐
- 그러나 두 개의 `for..of` 반복문을 하나의 객체에 동시에 사용할 수 없음
...
- 배열 메서드 사용 가능
- 인수가 이터러블이나 유사 배열인 경우
  - 새로운 배열 생성
  - 객체의 모든 요소를 새롭게 만든 배열로 복사
- `mapFn`
  - 각 요소를 대상으로 `mapFn` 적용하고 반환된 값을 새로운 배열에 추가
- `thisArg`
  - 각 요소의 `this`를 지정
