...
last_modified_at: 2023-12-06 06:03:11 +0900
...
- 함수의 정의 방법과 상관 없음
- 받는 인수의 개수보다 더 넘겨줘도 에러가 발생하지 않음
...
- 화살표 함수에서 `arguments` 객체에 접근하면, 외부의 일반 함수의 `arguments` 객체 가져옴
- 자체 `this`를 갖지 않음
- `arguments` 객체도 지원하지 않음
...
`Array.from(obj)`, `Array.from(iter)`

- 유사 배열 객체, 이터러블 객체 둘 다에 사용 가능
