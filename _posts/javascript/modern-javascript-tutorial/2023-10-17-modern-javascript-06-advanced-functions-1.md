...
last_modified_at: 2023-11-28 12:58:57 +0900
...
- 현재 함수의 실행 일시 중지
- 중지된 함수와 연관된 실행 컨텍스트는 실행 컨텍스트 스택(execution context stack)이라는 특별한 자료구조에 저장됨
- 중첩 호출 실행
- 중첩 호출 실행이 끝난 이후 실행 컨텍스트 스택에서 일시 중단한 함수의 실행 컨텍스트 꺼내오고 중단한 함수의 실행 재개

1. 실행 컨텍스트 스택 최상단에 현재 컨텍스트가 기록됨
...
상당수의 자바스크립트 내장 함수는 인수의 개수에 제약이 없음
...
함수의 정의 방법과 상관 없이 함수에 넘겨주는 인수의 개수에는 제약 없음
