...
1. 스크립트가 시작되면 스크립트 내에서 선언한 변수 전체가 렉시컬 환경에 올라감(pre-populated)
   ...
   - 자바스크립트 엔진은 uninitialized 상태의 변수를 인지하나, `let`을 만나기 전까지 해당 변수 참조 불가능
2. `let phrase`가 나타나고, 아직 값을 할당하기 전이기 때문에 프로퍼티 값은 `undefined`
...
- 이제 `counter()`의 렉시컬 환경이 참조하는 외부 렉시컬 환경에서 `count`를 탐색하고 발견
  - `count++`가 실행되면서 `count` 값이 1 증가하는데, 변숫값 갱신은 변수가 저장된 렉시컬 환경에서 이뤄짐
...
let g = f(); // g가 살아있는 동안엔 연관 렉시컬 환경도 메모리에 유지됨
...
let x = 1;
function func() {
  // 지역 변수 x는 함수 시작부터 엔진이 알고 있으나 let을 만나기 전까지 uninitialized(unusable) 상태
  console.log(x);
