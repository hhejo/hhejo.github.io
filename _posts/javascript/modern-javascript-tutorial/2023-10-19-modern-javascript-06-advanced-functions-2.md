...
last_modified_at: 2024-05-02 15:37:08 +0900
...
- 수행하는 코드를 한데 묶어 블록 안에는 작업 수행에만 필요한 변수가 들어감
- `if`, `for,` `while`에서도 동일
  - `for (let i = 0; ...)`에서 `let i`는 코드 블록 밖에 있어 보이나, 블록에 속하는 코드로 취급
...
alert(message); // ReferenceError: message is not defined (블록 내 변수에 접근 불가)
...
alert(phrase); // ReferenceError: phrase is not defined (블록 내 변수에 접근 불가)

// 4
for (let i = 0; i < 3; i++) {
  alert(i); // 0, 1, 2
}
alert(i); // ReferenceError: i is not defined (블록 내 변수에 접근 불가)
```
...
- 새로운 객체의 프로퍼티 형태나 중첩 함수 그 자체로 반환될 수 있음
  - 반환된 중첩 함수는 어디서든 호출해 사용할 수 있고 외부 변수에 접근 가능
- 코드를 정돈하는 데에 사용할 수 있음
...
alert(counter()); // 1
alert(counter()); // 2
let counter2 = makeCounter(); // counter와 독립적
alert(counter2()); // 0
```
...
- 명세서에서 자바스크립트가 어떻게 동작하는지 설명하는 데 쓰이는 이론상의 객체
  - 렉시컬 환경은 명세서에만 존재해 코드를 사용해 직접 렉시컬 환경을 얻거나 조작할 수 없음
...
- 그래서 렉시컬 환경이 만들어지는 즉시 사용 가능해 선언되기 전에도 함수 사용 가능
...
- 새로운 렉시컬 환경 자동 생성
- 호출 중인 함수를 위한 내부 렉시컬 환경 구성
- 함수 호출시 넘겨받은 매개 변수, 함수의 지역 변수
- 외부 렉시컬 환경에 대한 참조
...
- 내부 렉시컬 환경에서 원하는 변수를 찾지 못하면 내부 렉시컬 환경이 참조하는 외부 렉시컬 환경으로 검색 범위 확장
...
- `makeCounter()` 호출할 때마다 새로운 렉시컬 환경 객체 생성
...
- 자바스크립트에서는 모든 함수가 자연스럽게 클로저가 됨 (예외도 있음)
...
- 함수에 대응하는 렉시컬 환경이 메모리에서 제거되어 함수와 관련된 변수들 모두 사라짐
...
- 함수가 살아있는 동안에는 이론상으로 모든 외부 변수 역시 메모리에 유지됨
...
let g = f();
g(); // 이론상으로는 value에 접근할 수 있어야 하지만 최적화 대상이 되어서 에러 발생
...
```javascript
function makeWorker() {
  let name = "Pete"; // 이 코드가 없었다면 John 출력
  return function () {
    alert(name);
  };
}
...
let counter = makeCounter(); // 두 함수는 각각 다른 makeCounter 호출에 의해 생성
let counter2 = makeCounter(); // 두 함수는 독립적인 렉시컬 환경을 가져 자신만의 count를 가짐
alert(counter()); // 0
alert(counter()); // 1
alert(counter2()); // 0
alert(counter2()); // 1
...
alert(counter.count); // undefined (count 변수에 직접 접근 불가)
counter; // Counter { up: [Function (anonymous)], down: [Function (anonymous)] }
let counter2 = new Counter();
alert(counter2.up()); // 1
alert(counter2.up()); // 2
alert(counter2.down()); // 1
alert(counter2.count); // undefined (count 변수에 직접 접근 불가)
counter2; // Counter { up: [Function (anonymous)], down: [Function (anonymous)] }
```

- 생성자 함수의 두 중첩 함수 up, down은 동일한 외부 렉시컬 환경에서 생성되어 같은 `count` 변수를 공유
...
if (true) {
  ...
  let testLet = true; // testLet은 블록 내에서만 접근 가능
}
alert(testVar); // true
...
- `var`로 선언한 변수: 어디서든 참조 가능하고 무언가를 할당하기 전까지는 값이 `undefined`
...
- `var`로 선언한 모든 변수는 함수의 최상위로 끌어올려지기(hoisted) 때문
  - 함수 실행이 시작될 때 변수 선언 처리
- 선언은 호이스팅 되지만 할당은 호이스팅 되지 않고 할당 관련 코드에서 처리됨
...
- 과거에 `var`만 사용 가능했을 때 고안된 방법으로, `var`도 블록 레벨 스코프를 갖게 함
...
자바스크립트가 함수 표현식이라고 인식하게 해주는, IIFE를 만드는 방법들
