...
last_modified_at: 2023-12-06 15:33:06 +0900
...
- 함수 안에 있지 않은(not inside the function)
  - 자바스크립트에서 전역의 의미
...
var gVar = 5;
alert(window.gVar); // 5. 전역 객체 window의 프로퍼티가 됨. 권장하지 않는 사용법
let gLet = 5;
...
- `name` 프로퍼티로 가져올 수 있음
- 함수처럼 자동 할당 되지 않음
- 적절한 이름을 추론하는 게 불가능한 상황일 때
  - `name` 프로퍼티에 빈 문자열이 저장됨
...
function testFunc(printName = function () {}) {
  alert(printName.name); // 기본값으로 이름 할당
}
testFunc(); // printName
...
- 프로퍼티를 저장하는 것처럼 함수를 객체처럼 다룰 수 있음
  - 그러나 이는 실행에 아무 영향을 끼치지 않음
  - 변수는 함수 프로퍼티가 아님
  - 함수 프로퍼티는 변수가 아님
- 클로저는 함수 프로퍼티로 대체 가능
...
- 주요 함수를 하나 만들고, 여기에 다양한 헬퍼 함수를 붙이는 식으로 라이브러리를 구성
  - 라이브러리 하나가 전역 변수 하나만 사용하므로 전역 공간을 더럽히지 않음
  - 이름 충돌도 방지
...
- 함수 `counter`에 할당된 메서드들은 동일한 외부 렉시컬 환경을 공유
  - 동일한 `count`에 접근 가능
...
- 어떤 문자열도 함수로 변경 가능
  - 서버에서 전달받은 문자열을 이용해 새로운 함수를 만들고 실행
...
- 문자열을 사용해서 함수를 만드는 경우, 서버를 비롯한 외부에서 코드를 받아올 수 있음
  - 만들어진 새 함수는 기존 스크립트와 문제 없이 상호작용할 수 있어야 함
...
- 함수 내부에 `let userName`이라는 지역 변수가 있을 때
...
- 압축기가 동작한 이후에 `new Function`으로 만든 함수의 내부에서 외부 렉시컬 환경에 접근하려고 할 때 문제가 발생할 수 있음
- 함수 내부에서 외부 변수에 접근하는 것은 아키텍처 관점에서도 좋지 않고 에러에 취약

압축기(minifier)

- 스크립트에서 주석이나 여분의 공백 등을 없애 코드 크기를 줄여주는 프로그램
- 단순히 변수를 찾아서 바꾸지 않음
