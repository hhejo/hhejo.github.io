...
last_modified_at: 2023-12-06 23:01:29 +0900
...
- 특정 프로퍼티에 값을 쓰지 못하도록(non-writable) 할 수 있음
- `false`인데 값을 쓰는 경우
- 엄격모드: 에러 발생
- 비 엄격모드: 값이 변경되지 않음
...
- `false`인 경우 `for..in` 반복문에 해당 프로퍼티가 나타나지 않음
- 객체 내장 메서드 `toString`은 열거가 불가능(non-enumerable)
  - `for..in` 사용 시 나타나지 않음
- 커스텀 `toString`을 추가하면 `for..in`에 나타남
- 열거 불가능한 프로퍼티는 `Object.keys`에서도 배제됨
...
- `false`로 설정된 경우, 해당 프로퍼티는 객체에서 지울 수 없음
...
- 접근자 프로퍼티를 사용하면 함수처럼 호출하지 않음
  - 일반 프로퍼티에서 값에 접근하는 것처럼 평범하게 프로퍼티 값을 얻을 수 있음
...
- `defineProperty`에 설명자 `get`, `set`을 전달해 특정 프로퍼티를 위한 접근자 생성 가능
...
- 데이터 프로퍼티(`value`를 가짐) 중 한 종류에만 속함
- 둘 다에 속할 수 없음
