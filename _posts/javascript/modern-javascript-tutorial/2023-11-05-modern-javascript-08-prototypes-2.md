...
last_modified_at: 2023-12-07 13:02:48 +0900
...
- `prototype` 프로퍼티는 자바스크립트 내부에서도 광범위하게 사용
...
- 내장 객체들 역시 프로토타입에 메서드 저장됨
  - `Array`, `Date`, `Function` 등
- 명세서에서는 모든 내장 프로토타입의 상속 트리 꼭대기에는 `Object.prototype`이 있어야 한다고 규정
- 모든 것은 객체를 상속받는다고 하기도 함

배열 `[1, 2, 3]` 생성하는 경우

- `new Array()`의 디폴트 생성자가 내부에서 동작
- 배열 `[1, 2, 3]`의 프로토타입이 `Array.prototype`이 됨
- `Array.prototype`을 통해 배열 메서드 사용 가능
- 이런 내부 동작으로 메모리 효율 상승

체인 상의 프로토타입에 중복 메서드가 있을 때

- 체인 상에서 가까운 곳에 있는 메서드 사용
- `Array.prototype`의 자체 메서드 `toString`이 있음
  - 요소 사이에 쉼표를 넣어 요소 전체를 합친 문자열을 반환
- `Object.prototype`에도 메서드 `toString`이 있음

배열이 아닌 다른 내장 객체들 또한 같은 방법으로 동작

- 함수는 내장 객체 `Function`의 생성자로 생성
- 함수에서 사용할 수 있는 메서드는 `Function.prototype`에서 받아옴
  - `call`, `apply` 등
...
- 문자열, 숫자, 불린값은 객체가 아님
- 그런데 이런 원시 타입 값의 프로퍼티에 접근할 경우 임시 래퍼(wrapper) 객체 생성
  - 내장 생성자 `String`, `Number`, `Boolean`을 사용
- 임시 래퍼 객체는 이런 메서드를 제공한 후 사라짐
- 래퍼 객체는 보이지 않는 곳에서 생성됨
  - 엔진에 의해 최적화됨
...
- 내장 메서드 `join`의 내부 알고리즘
  - 제대로 된 인덱스와 `length` 프로퍼티가 있는지만 확인
  - 호출 대상이 진짜 배열인지는 상관 없음
- 다수의 내장 메서드가 이런 식으로 동작
- `obj.__proto__`를 `Array.prototype`으로 설정해 배열 메서드를 상속받게 할 수도 있음
  - `obj`에서 모든 `Array` 메서드를 사용할 수 있음
- 그런데 자바스크립트는 단일 상속만 허용
  - 이 방법은 `obj`가 다른 객체를 상속받고 있을 때는 사용할 수 없음
...
- 객체 메서드에 대한 데코레이션 동작을 만들기 위해서 `this`를 `f.apply` 안에서 사용
...
- `for..in`을 사용해 프로퍼티를 복사하는 것보다 더 효과적으로 객체 복제 가능
...
`Object.create`으로 효과적으로 객체 복제하기

- `for..in`으로 프로퍼티를 복사하는 것보다 효과적
- 열거 가능한 프로퍼티
- 열거 불가능한 프로퍼티
- 데이터 프로퍼티
- getter, setter
...
`[[Prototype]]`을 다루는 방법은 다양한데, 역사적인 이유가 있음

- 생성자 함수의 `prototype` 프로퍼티는 아주 오래전부터 사용됨
- 2012년 명세서에 `Object.create` 추가
  - 주어진 프로토타입으로 객체 생성
- 프로토타입을 얻거나 설정할 수는 없음
- 그래서 브라우저는 비표준 접근자 `__proto__`를 구현
  - 언제나 프로토타입을 얻거나 설정할 수 있게 함
- 이후 2015년 표준에 `Object.setPrototypeOf`, `Object.getPrototypeOf` 추가
  - `__proto__`와 동일한 기능 수행 가능
- 그런데 이 시점에는 `__proto__`를 사용하는 곳이 너무 많았음
  - `__proto__`는 사실상 표준(de-facto standard)이 됨
...
- 하지만 대개 객체를 생성할 때만 `[[Prototype]]`을 설정하고 이후에는 수정하지 않음
...
- 사용자가 직접 입력한 키를 가지고 객체를 만들다 보면 사소한 결함 발생
  - 커스텀 사전 만들기
- `"__proto__"`라는 문자열은 문자열 키로 사용할 수 없음
  - 프롬프트 창에 `__proto__`를 입력하면 값이 제대로 할당되지 않음
- `__proto__` 프로퍼티는 특별한 프로퍼티로, 객체나 `null`이어야 함
- 키가 `__proto__`일 때 값이 제대로 저장되지 않는 것은 버그
  - 키가 무엇이 되었든 키-값 쌍은 저장되어야 함
- 할당 값이 객체인 경우, 프로토타입이 변경될 수 있음
- 프로토타입이 변경되면 예상치 못한 일이 발생할 수 있음
...
- 객체 대신 맵을 사용
- `Object.create(null)`로 프로토타입이 없는 빈 객체 생성
  - `[[Prototype]]`이 `null`인 객체
  - `__proto__` getter와 setter를 상속받지 않음
  - `Object.keys` 등 메서드는 사용 가능
...
- `__proto__`는 `[[Prototype]]`에 접근하기 위한 수단
  - `[[Prototype]]` 그 자체가 아님
