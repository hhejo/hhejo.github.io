...
last_modified_at: 2023-12-10 08:37:02 +0900
...
- `try..catch`로 스크립트가 죽는 것을 막고 에러를 잡음
  - 더 합당한 무언가를 할 수 있음
...
2. 에러가 없다면 `try` 안의 마지막 줄까지 실행
   - `catch` 블록은 건너뜀
3. 에러가 있다면 `try` 안 코드의 실행이 중단
   - `catch (err)` 블록으로 제어 흐름 넘김
   - `err`는 에러 객체를 포함
   - 아무 이름 가능
...
- `stack`
  - 가장 널리 사용되는 비표준 프로퍼티
  - 대부분의 호스트 환경에서 지원하며 디버깅 목적으로 사용
  - 현재 호출 스택
...
- 이 생성자들로 에러 객체 생성 가능
- 내장 에러 객체의 `name` 프로퍼티는 생성자 이름과 동일
  - 일반 객체가 아닌 내장 생성자를 사용해 만든 경우
...
- `finally`라는 코드 절을 가질 수 있음
- `finally` 안의 코드는 다음과 같은 상황에서 실행
...
- 전역 핸들러
- 여기에 함수를 할당하면 예상치 못한 에러가 발생했을 때 실행됨
- `message`: 에러 메시지
...
- 그러나 `Error`를 상속받아 커스텀 에러 클래스를 만드는 경우
- `obj instanceof Error`로 에러 객체 식별 가능
