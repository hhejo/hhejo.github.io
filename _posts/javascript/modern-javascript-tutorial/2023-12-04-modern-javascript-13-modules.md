...
last_modified_at: 2023-12-10 20:08:34 +0900
...
- 모듈 스크립트는 HTML 문서가 완전히 준비될 때까지 대기 상태
- HTML 문서가 완전히 만들어진 이후 실행
...
```html
<!-- 모듈 스크립트 -->
<!-- 지연 실행되기 때문에 페이지가 모두 로드된 후 alert 함수 실행 -->
<script type="module">
  alert(typeof button); // (2) object. 아래 쪽의 button 요소를 정상적으로 출력
</script>

<!-- 일반 스크립트 -->
<!-- 페이지가 완전히 구성되기 전이라도 바로 실행 -->
<script>
