...
last_modified_at: 2024-01-07 09:36:34 +0900
...
- DOM은 브라우저만을 위한 모델이 아님
  - 브라우저가 아닌 곳에서도 DOM을 사용하는 경우가 있음
- `document`: 페이지의 기본 진입점 역할 객체
  - `document` 객체로 페이지 내 무엇이든 변경·생성 가능
...
- 자식 노드를 가질 수 없음. 트리의 끝에서 leaf 노드가 됨
...
1. `<head>` 이전의 공백과 새 줄은 무시됨
   - 역사적인 이유로 인함
2. `</body>` 뒤에 무언가를 넣어도 자동으로 `body` 안쪽으로 옮겨져 `</body>` 뒤에는 공백이 있을 수 없음
   - HTML 명세서에서 모든 컨텐츠는 `body` 안쪽에 있어야 함

공백 없는 텍스트 노드만으로 HTML 문서를 구성하기
...
- 최상위 태그는 항상 `<html>`
  - 없으면 자동으로 문서 최상위에 넣어줌
- `<body>`도 동일한 방식 적용
- 테이블에는 언제나 `<tbody>` 존재
  - DOM 명세서에는 테이블에 반드시 `<tbody>`가 있어야 한다고 명시
  - HTML에서는 `<tbody>` 생략하곤 함
...
- 주석도 노드가 되고 DOM에 추가됨
- 화면에는 출력되지 않으나 HTML에 있다면 반드시 DOM 트리에 추가되어야 하는 규칙
...
- HTML 문서 최상단의 `<!DOCTYPE...>` 지시자도 DOM 노드가 됨
  - `<html>` 바로 위에 위치
...
- 브라우저가 스크립트를 읽는 도중에 아직 읽지 않은 요소
- 존재하지 않는 요소로 스크립트에서 접근 불가
...
`nextSibling`, `previousSibling` 프로퍼티
...
요소 `id` 속성값을 따서 자동으로 선언된 전역변수

- 동일한 이름을 가진 변수가 선언되면 무용지물
- 하위 호환성을 위해 남긴 동작이라 요소 접근 시 사용하지 말 것
- `getElementById` 같은 메서드 권장
...
- 해당하는 요소를 찾으면 검색을 멈춤
  - `elem.querySelectorAll(css)[0]`보다 더 빠르고 코드가 짧음
