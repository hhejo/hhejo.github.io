...
last_modified_at: 2024-01-05 15:07:55 +0900
...
- margin은 요소 자체에 포함되지 않고 관련 특수 자바스크립트 프로퍼티도 없음
- 요소 내 텍스트가 길어 넘치면 브라우저가 이 텍스트들을 `padding-bottom`에 표시
...
- 테두리를 포함한 요소 전체의 사이즈 정보 제공

화면에 표시되지 않는 요소의 기하 프로퍼티 값은 `0` 또는 `null`
...
`getComputedStyle`로 요소 너비와 높이를 얻지 말 것

- 대신 기하 프로퍼티를 사용해 너비와 높이 정보를 얻어야 함
...
`document.documentElement`는 `<html>` 태그와 상응하는 요소로 다양한 메서드 지원
...
- 눈에 보이는 문서에서 컨텐츠가 실제로 들어가게 될 영역의 너비와 높이 값 반환
...
- 브라우저 상관 없이 쓸 수 있는 더 편한 대안
- 브라우저 종류에 상관 없이 동일한 동작을 보장
- `scrollBy(x, y)`: 페이지의 스크롤 상태를 현재 포지션을 기준으로 상대적으로 조정
  - `scrollBy(0, 10)`로 10px 스크롤 내림
- `scrollTo(pageX, pageY)`: 절대 좌표를 기준으로 페이지 스크롤 상태 변경
  - 눈에 보이는 영역의 왼쪽 위 모서리의 좌표가 문서 전체의 왼쪽 위 모서리를 기준으로 `(pageX, pageY)`가 됨
  - `scrollTo(0, 0)`으로 문서 스크롤 상태를 처음으로 되돌릴 수 있음
...
- `top`이 `true`(디폴트)인 경우, `elem`이 창 제일 위에 보이도록 스크롤 상태가 변경됨
  - `elem`의 위쪽 모서리가 창의 위쪽 모서리와 일치
- `top`이 `false`인 경우, `elem`이 창 가장 아래에 보이도록 스크롤 상태가 변경됨
...
- 스크롤바가 사라지는 단점 존재
- 스크롤바는 일정 공간을 차지하는데, 스크롤바가 사라지면 해당 공간을 채우기 위해 컨텐츠가 갑자기 움직이는 현상 발생
- 이렇게 되면 사용자 입장에서 이상해 보일 수 있음
- 스크롤바를 고정시키기 전과 후의 `clientWidth` 값을 비교해 해당 증상을 보정해야 함
- 스크롤바가 사라질 때는 `clientWidth` 값이 갑자기 커짐
- 이때 스크롤바가 차지했던 영역만큼 `document.body`에 `padding`을 줘 컨텐츠 전체의 너비를 스크롤바가 사라지기 전과 같은 값으로 유지 가능

요즘 모바일 웹은 스크롤을 조금 내릴 경우, 전체화면 모드로 전환(도구 막대 사라짐)되면서 창 크기가 달라짐
...
- `pageY`: 문서 맨 위부터 계산되기 때문에 스크롤 전, 후 값 동일
- `clientY`: 해당 지점이 창 상단과 가까워졌기 때문에 창 기준 좌표가 변함
