...
last_modified_at: 2024-01-13 06:12:08 +0900
...
- 클릭과 연관된 이벤트가 정확히 어떤 버튼에서 이벤트가 발생했는지 알려줌
- `click`, `contextmenu`는 왼쪽, 오른쪽 버튼을 눌렀을 때 발생하기 때문에 잘 사용하지 않음
...
- MacOS에서는 `Ctrl` 대신 `Cmd`를 사용할 것
  - `Ctrl` 키와 마우스 왼쪽 버튼을 함께 누른 경우, 마우스 오른쪽 버튼을 누른 것으로 해석
...
- `mouseover`, `mouseout`에 있는 특별한 프로퍼티
- `target` 프로퍼티를 보완
- 마우스가 한 요소에서 다른 요소로 이동하면, 그 중 하나는 `target`이 되고, 그 중 하나는 `relatedTarget`이 됨
- `null`이 될 수 있음
  - 마우스가 다른 요소가 아닌 창 밖에서 나왔거나 창 밖으로 떠났다는 것을 의미
  - `event.relatedTarget.tagName`처럼 쓰면 에러가 발생
...
- 포인터가 창 밖에서 페이지 중앙 안쪽으로 바로 점프하는 것이 가능
...
- `mouseout`: target이 parent
- `mouseover`: target이 child (이벤트 버블링)

### Events mouseenter and mouseleave

`mouseenter`, `mouseleave` 이벤트가 `mouseover`, `mouseout`과 다른 점

- 요소 내부의 하위 항목 간 전환은 계산되지 않음
- 이벤트가 버블링되지 않음
