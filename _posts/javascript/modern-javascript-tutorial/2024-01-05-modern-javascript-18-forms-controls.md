...
- `document.forms`: 특수한 컬렉션으로 폼(form)이 이것의 구성원
  - 이름과 순서가 있는 기명 컬렉션(named collection)
- `form.elements`: 폼의 요소 얻기
- `form.elements[name]`: 컬렉션. 라디오 버튼 등 이름이 같은 요소 여러 개 다루는 경우 사용
- `fieldset`: 하위 폼처럼 쓰임. `elements` 프로퍼티 지원
  - 폼은 하나 이상의 `<fieldset>` 요소를 포함 가능
- `form.name`: `form.elements.name` 대신 짧은 표기법. `form[index/name]`으로도 요소에 접근 가능
  - 요소의 `name` 속성을 변경해도 변경 전 이름을 계속 사용할 수 있는 문제가 있음
  - 폼 요소의 이름을 변경하는 일은 드물어 보통은 문제되지 않고 새로운 이름도 물론 사용 가능
...
<script>
  ...
  form = document.forms[0]; // 문서 내 첫 번째 폼
...
<script>
  ...
  alert(form.elements.login); // undefined (form.elements에는 name 속성 변경이 반영됨)
  alert(form.elements.username); // input
  alert(form.username == form.login); // true (form은 새로운 이름과 이전 이름을 모두 인식)
</script>
```

### element.form으로 역참조하기

- `element.form`: 모든 요소는 `element.form`으로 폼에 접근 가능
...
- input과 textarea 요소의 값 얻기
- `input.value`(string), `input.checked`(boolean)
- `textarea.value`, `textarea.innerHTML`
- `textarea.innerHTML`은 페이지를 처음 열 당시의 HTML만 저장되어 최신 값을 구할 수 없기 때문에 사용하지 말 것
- select와 option
...
- `<select>`의 값 설정하는 세 가지 방법
  1. 조건에 맞는 `<option>` 하위 요소를 찾아 `option.selected` 속성을 `true`로 설정 (가장 확실)
  2. `select.value`를 원하는 값으로 설정
  3. `select.selectedIndex`를 원하는 option 번호로 설정 (2, 3번 방법이 더 편함)
...
- `elem.focus()`, `elem.blur()`: 요소에 포커스를 주거나 제거
...
- 포커스는 손실은 많은 이유로 발생하고 자바스크립트 자체가 이를 유발할 수 있음
  - `alert`는 포커스를 자신에게 옮기고 해제되면 되돌아감
  - 요소가 DOM에서 제거되면 포커스 손실이 발생하고, 다시 삽입돼도 포커스가 돌아오지 않음
- 이러한 기능으로 `focus`, `blur` 핸들러가 오작동할 수 있기 때문에 주의해서 사용
...
- 속성값은 숫자로 Tab 키를 눌러 요소 사이를 이동할 때의 순서
- `tabindex`가 없는 요소는 문서 내 순서에 따라 포커스 이동

`tabindex` 주의 사항

- `tabindex`가 `0`인 요소: `tabindex` 속성이 없는 것처럼 동작
...
- `tabindex`가 `-1`인 요소: 스크립트로만 포커스 하고 싶은 요소에 사용
...
- `1` -> `2` -> `0` 순서로 이동. `-1`은 무시됨

### focusin과 focusout을 사용해 이벤트 위임하기

- `focus`, `blur` 이벤트: 버블링 되지 않음
  - 이벤트 위임 효과를 주는 방법 2가지
  - `focus`, `blur`는 버블링 되지 않지만 캡처링은 됨
  - `focusin`, `focusout` 사용하기
- `focusin`, `focusout` 이벤트: `focus`, `blur`와 동일하나 버블링 됨
  - `on<event>` 방식이 아닌 `elem.addEventListener` 방식으로 핸들러 추가
...
- 인풋 필드에서 Enter 키 누르기. 이때 `click` 이벤트도 트리거 됨
