...
last_modified_at: 2024-01-14 09:22:38 +0900
...
- 태스크가 들어오길 기다리고, 태스크가 들어오면 처리하고, 없으면 잠듦
...
- 이때 세 태스크는 큐에 하나씩 추가됨
- 큐에 있는 태스크들은 들어간 순서대로 처리됨
  - `script` -> `mousemove` -> `setTimeout`
...
- 현재 작업 중인 태스크가 끝나야 DOM 변경분을 화면에 렌더링
  - 시간이 오래 걸리든 아니든 상관 없이
...
- 작업 진척 상태를 보여주는 인디케이터를 만들어야 하는 경우
  - 프로그레스 바 등
...
1. 매크로태스크 큐에서 가장 오래된 태스크를 꺼내 실행
   - 스크립트 실행 등
2. 모든 마이크로태스크를 실행
   - 마이크로태스크 큐가 빌 때까지 이어지고, 태스크는 오래된 순서대로 처리
3. 렌더링할 것이 있으면 처리
