...
last_modified_at: 2024-01-16 19:48:29 +0900
...
`ArrayBuffer`는 무언가의 배열이 아님

- `ArrayBuffer`는 `Array`와 공통점이 없음
- 길이가 고정되어 있으므로 늘리거나 줄일 수 없음
- 메모리에서 정확히 그만큼의 공간을 차지
- 개별 바이트에 접근하려면 `buffer[index]`가 아닌 다른 뷰(view) 객체가 필요

`ArrayBuffer`는 메모리 영역

- 이 안에 무엇이 저장되어 있는지는 전혀 단서가 없음. 단지 원시 바이트 시퀀스
- `ArrayBuffer`를 조작하려면 뷰(view) 객체를 사용
- 뷰 객체는 자체적으로 아무것도 저장하지 않음
- `ArrayBuffer`에 저장된 바이트를 해석하는 것은 eyeglasses

예를 들어

- `Uint8Array`: `ArrayBuffer`에 있는 각 바이트를 별도의 숫자로 처리
  - 가능한 값은 0~255(바이트는 8비트이므로 그만큼만 저장 가능)
  - 이러한 값을 부호 없는 8비트 정수(8-bit unsigned integer)라고 함
- `Uint16Array`: 2바이트마다 0~65535의 값을 갖는 정수로 처리 (부호 없는 16비트 정수)
...
- 이러한 뷰에 대한 일반적인 용어
- 이들은 동일한 메서드 및 프로퍼티 집합을 공유
- `TypedArray`라는 생성자는 없음
- 그저 뷰 중 하나를 나타내는 일반적인 umbrella 용어
- `new TypedArray` 같은 것은 `new Int8Array`, `new Uint8Array` 등을 의미
- typed array는 일반 배열처럼 동작(인덱스가 있고 반복 가능)
- type array 생성자는 인수 타입에 따라 다르게 동작
- 인수에는 5가지 변형이 있음
...
- `byteOffset`: `byteOffset`부터 `length`까지 뷰가 `buffer`의 일부만 포함하도록 할 수 있음
- 기본값은 0부터 버퍼 끝까지
...
alert(arr.length); // 4 (같은 길이의 바이너리 배열 생성)
alert(arr[1]); // 1 (주어진 값들을 4바이트로 채움(부호 없는 8비트 정수))
```

3 다른 `TypedArray`가 제공되면 동일한 작업을 수행

- 즉, 동일한 길이의 형식화된(typed) 배열을 만들고 값을 복사
...
alert(arr8[0]); // 1
alert(arr8[1]); // 232 (1000을 복사하려 했지만, 1000을 8비트에 맞출 수 없음)
```
...
```javascript
let arr = new Uint16Array(4); // 4개 정수에 대한 형식화된 배열 생성
...
`ArrayBuffer`에 접근하려면 다음 프로퍼티가 있음

- `arr.buffer`: `ArrayBuffer`에 대한 참조
- `arr.byteLength`: `ArrayBuffer`의 길이

따라서 항상 한 뷰에서 다른 뷰로 이동 가능

```javascript
let arr8 = new Uint8Array([0, 1, 2, 3]);
let arr16 = new Uint16Array(arr8.buffer); // 동일한 데이터에 대한 다른 뷰
```
...
- 256을 `Uint8Array`에 넣는 경우, 256은 바이너리 형식으로 `100000000`(9비트)이지만 `Uint8Array`는 값당 8비트를 제공하므로 사용 가능한 범위는 0~255
- 더 큰 숫자의 경우, 가장 오른쪽(rightmost, less significant) 8비트만 저장되고 나머지는 잘림
- 따라서 0을 얻음
...
- 이 측면에서 특별하고 동작이 다름
- 255보다 큰 숫자에 대해서는 255를 저장하고, 음수에 대해서는 0을 저장
- 이 동작은 이미지 처리에 유용
...
- 할 수 없는 일도 몇 가지 있음
- `splice` 불가: 값을 삭제할 수 없음. typed array는 버퍼에 대한 뷰이고, 고정된 연속 메모리 영역이기 때문
  - 우리 할 수 있는 일은 0을 할당하는 것뿐
- `concat` 불가
- 2가지 추가 메서드가 있음
- `arr.set(fromArr, [offset])`: `fromArr`에서 `arr`로 모든 요소 복사
  - `offset` 위치에서 시작(기본값은 0)
...
- `ArrayBuffer`에 대한 특별하고 매우 유연한 untyped 뷰
...
- `label`: 기본적인 인코딩 방식은 `utf-8`
  - `big5`, `windows-1251` 및 다른 인코딩 방식도 지원
- `options`: 선택 항목
  - `fatal`: `true`인 경우, 잘못된 글자(디코딩 불가능한 글자)를 대상으로 예외를 던짐. `false`(기본값)인 경우, 글자를 `\uFFD`로 대체
...
alert(new TextDecoder().decoder(uint8Array)); // Hello
```

버퍼의 하위 배열 뷰를 생성해 버퍼의 일부를 디코딩할 수 있음

```javascript
let uint8Array = new Uint8Array([0, 72, 101, 108, 111, 0]); // 문자열을 나타내는 배열의 요소가 중간에 존재
let binaryString = uint8Array.subarray(1, -1); // 배열의 복사 없이 문자열 출력
...
- `encode(str)`: `Uint8Array`에 문자열을 반환
...
- 선택적 문자열 `type`(일반적으로 MIME 타입)
- 다른 `Blob` 객체들의 시퀀스 `blobParts`
- 문자열
- `BufferSource`
...
- `options`: 선택적 객체
  - `type`: `Blob` 타입. 일반적으로 MIME 타입
    - `image/png` 등
  - `endings`: `Blob`이 현재 OS 개행문자(`\r\n`, `\n`)에 해당하도록 end-of-line을 변환할지 여부
    - 기본값은 `transparent`로, 아무것도 하지 않음
    - `native`는 변환한다는 뜻
...
let blob = new Blob([hello, " ", "world"], { type: "text/plain" }); // typed array와 문자열로 Blob 생성
```
...
- `link.href`의 값은 위와 같이 보일 것

`URL.createObjectURL`로 생성된 각 URL

브라우저는 내부적으로 URL -> `Blob` 매핑을 저장

따라서 이런 URL들은 짧지만 `Blob`에 접근할 수 있게 해줌
...
`Blob`에 대한 매핑이 있는 동안, `Blob` 자체는 메모리에 남아있음
...
- revocation 후 매핑이 제거되므로 URL은 더이상 작동하지 않음
...
`Blob`을 base64로 변환하기 위해 `FileReader` 내장 객체를 사용
